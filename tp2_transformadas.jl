...
function transformarMatriz(matrix)
	...
	for i in 1:8:ancho
		for j in 1:8:alto
			k = i
			l = j
			matrix[i:i+7,j:j+7]  = dct(matrix[k:k+7,l:l+7])
			end
		end
	return matrix
end
...
# inversa
function transformarInversaMatriz(matrix)
	alto  = length(matrix[1,:])
	ancho = length(matrix[:,1])
	print(ancho)
	for i in 1:8:ancho
		for j in 1:8:alto
			k = i
			l = j
			matrix[i:i+7,j:j+7]  = idct(matrix[k:k+7,l:l+7])
			end
		end
	return matrix
end

# ╔═╡ 7390b45f-bb84-4b7a-9464-c9adbcc2d10e
length(matrix[1,:])

# ╔═╡ bca20ac7-1f75-42dd-99c6-01e3159d505c
transformarMatriz(descompuestaPing[1])

# ╔═╡ bd702b58-6814-43be-a7fa-087c3902222c
recomposicionRGB(transformarMatriz(descompuestaPing[1]), transformarMatriz(descompuestaPing[2]), transformarMatriz(descompuestaPing[3]))

# ╔═╡ 290a194a-756f-4cb6-8b59-e8fe285b7083
recomposicionRGB(transformarInversaMatriz(descompuestaPing[1]), 						transformarInversaMatriz(descompuestaPing[2]), 		transformarInversaMatriz(descompuestaPing[3]))
